...
class InferenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        true_C = self.C_vals[idx]
        true_A = self.A_vals[idx]
        true_T = self.T_vals[idx]
        true_H = self.H_vals[idx]
        
        return x, true_C, true_A, true_T, true_H


C_model = CNN(num_classes=4).to(device)
C_model.load_state_dict(torch.load('cnn_model_best.pth', map_location=device, weights_only=False))
C_model.eval()

A_checkpoint = torch.load('cnn_A_hierarchical_best.pth', map_location=device, weights_only=False)
A_model = HierarchicalCNN(
    num_classes=A_checkpoint['num_A_classes'],
...
T_checkpoint = torch.load('cnn_T_hierarchical_best.pth', map_location=device, weights_only=False)
T_model = HierarchicalCNN(
    num_classes=T_checkpoint['num_T_classes'],
...
H_checkpoint = torch.load('cnn_H_hierarchical_best.pth', map_location=device, weights_only=False)
H_model = HierarchicalCNN(
    num_classes=H_checkpoint['num_H_classes'],
    num_context_features=3
).to(device)
H_model.load_state_dict(H_checkpoint['model_state'])
H_model.eval()
H_to_idx = H_checkpoint['H_to_idx']
...
for _, row in df.iterrows():
    ...
    if not os.path.exists(fp):
        continue
    if row['T'] not in T_to_idx or row['H'] not in H_to_idx:
        continue
    paths.append(fp)
...
T_labels = np.array([T_to_idx[t] for t in T_vals])
...
CA_both_correct = ((C_preds == C_true) & (A_preds == A_true)).sum()
CA_cascade_acc = CA_both_correct / len(C_true)

CAT_all_correct = ((C_preds == C_true) & (A_preds == A_true) & (T_preds == T_true)).sum()
CAT_cascade_acc = CAT_all_correct / len(C_true)

CATH_all_correct = ((C_preds == C_true) & (A_preds == A_true) & (T_preds == T_true) & (H_preds == H_true)).sum()
CATH_cascade_acc = CATH_all_correct / len(C_true)
...
print(f"  C+A both correct: {CA_cascade_acc:.4f}")
print(f"  C+A+T all correct: {CAT_cascade_acc:.4f}")
print(f"  C+A+T+H all correct: {CATH_cascade_acc:.4f}")
...
for c_idx in range(4):
    ...
    A_acc_c = accuracy_score(A_true[mask], A_preds[mask])
    T_acc_c = accuracy_score(T_true[mask], T_preds[mask])
    H_acc_c = accuracy_score(H_true[mask], H_preds[mask])
    print(f"  C={c_val}: A={A_acc_c:.4f}, T={T_acc_c:.4f}, H={H_acc_c:.4f} (n={mask.sum()})")
...
plt.tight_layout()
plt.savefig('inference_CATH.png', dpi=300)
print("\nSaved: inference_CATH.png")
...
results = {
    ...
    'T_accuracy': T_acc,
    'H_accuracy': H_acc,
    'CA_both_correct': CA_cascade_acc,
    'CAT_all_correct': CAT_cascade_acc,
    'CATH_all_correct': CATH_cascade_acc
}

pd.DataFrame([results]).to_csv('inference_CATH_results.csv', index=False)
print("Saved: inference_CATH_results.csv")
print("\nDone!")