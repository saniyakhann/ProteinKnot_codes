...
remaining_classes = np.unique(T_labels_sampled)
print(f"Classes after filtering (min {min_samples} samples): {len(remaining_classes)}")
print(f"Total samples: {len(T_labels_sampled)}")
...
train_loader = DataLoader(
    ...
    prefetch_factor=4, pin_memory=(device.type == 'cuda'), persistent_workers=True
)
...
checkpoint = torch.load("cnn_dist_T_best.pth", map_location=device, weights_only=False)
...
with torch.no_grad():
    for matrices, context, labels_batch in test_loader:
        ...
        all_C_vals.extend((context[:, 0] * 3.0 + 1).cpu().numpy().astype(int).tolist())
...
for c_val in [1, 2, 3, 4]:
    ...
    if mask.sum() == 0:
        continue
    c_acc = (all_preds[mask] == all_true[mask]).mean()
    print(f"  C={c_val}: {c_acc:.4f} (n={mask.sum()})")

print("\nClassification report:")
unique_classes = sorted(np.unique(np.concatenate([all_true, all_preds])))
target_names = [f"T{idx_to_T[i]}" for i in unique_classes]
...
plt.title("Confusion Matrix (Distance - T level)", fontsize=16)
plt.xlabel("Predicted", fontsize=12)
plt.ylabel("True", fontsize=12)
plt.tight_layout()
plt.savefig("confusion_matrix_dist_T.png", dpi=200)
plt.close()
...
plt.tight_layout()
plt.savefig('training_curves_dist_T.png', dpi=300)
plt.close()
