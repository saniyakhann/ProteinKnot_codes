...
print(classification_report(all_true, all_preds, labels=unique_classes, target_names=target_names, zero_division=0))

cm = confusion_matrix(all_true, all_preds)
plt.figure(figsize=(18, 16))
sns.heatmap(cm, annot=False, cmap='Blues', fmt='d')
plt.title(f"Confusion Matrix (Distance - H level, {len(unique_classes)} classes)", fontsize=16)
plt.xlabel("Predicted", fontsize=12)
plt.ylabel("True", fontsize=12)
plt.tight_layout()
plt.savefig("confusion_matrix_dist_H.png", dpi=200)
plt.close()
