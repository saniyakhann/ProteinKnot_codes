...
csv_path = 'T_level_domain_ids_writhe.csv'
...
df = df.merge(proteins_df[['domain_id', 'C', 'A', 'T']], on='domain_id', how='left')
...
t_to_idx = checkpoint['T_to_idx']
idx_to_t = checkpoint['idx_to_T']

model = HierarchicalCNN(num_classes=checkpoint['num_T_classes']).to(device)
...
for i, row in df.iterrows():
    ...
    true_t = int(row['T'])
    true_idx = t_to_idx.get(true_t, -1)
    if true_idx == -1:
        continue
    ...
    context = torch.tensor([[c_norm, a_norm]], dtype=torch.float32).to(device)
...
res_df.to_csv('gradcam_results_writhe_T.csv', index=False)
print('saved: gradcam_results_writhe_T.csv')
